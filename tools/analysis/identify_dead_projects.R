...
for(pid in pids){
  ...
  project.data.LOC <- max(project.data.last_months$LOC) - min(project.data.last_months$LOC) # If dead, the LOC diff will be low

  project.is_dead.by.LOC <- (project.data.LOC <= dead.threshold.LOC)
