...
The OhlohAnalytics tool by \citet{ohlohanalytics} performed initial analysis,
...
representing the \textit{fraction of continuity} of the evolution data per
...
\textit{similar sequences}. A sequence is defined as a subsequent series of
...
\paragraph{}
A distinction is made between the \textit{detection} of a pattern and an
\textit{occurrence} of a pattern. Although the difference is slight, the
distinction is essential for further analysis.

When \textit{detecting} a pattern in project $A$, it means it was
encountered by analysing that particular project. Finding an
\textit{occurrence} of a pattern means that the pattern was found by evaluating
other project $B$'s sequences against those from project $A$. A pattern is then
recorded as a tuple of a sequence detected in project $A$ and a set of at least
2 other sequences being similar, occurring elsewhere.
...
\begin{description}
	\item[Type A.] Type A patterns strictly detected at the end of the evolution
		data of a dead project. More formally, if a dead project $P$ starts at point
		$x$ and ends at point $y$, the pattern starts at $x'$ and ends at $y$.
	
	\item[Type B.] Type B patterns strictly \textit{not} detected at the end of
