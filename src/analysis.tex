...
To be able to know what the patterns represent, a sample of the patterns are
...
As the tuple of values of level of decomposition, starting point in the series,
the pattern length (i.e., number of coefficients), and the originating project
is unique for each pattern, it can be 'recomposed' to the original signal of
that particular project. This gives the $age\_in\_months$ values the sequence
starts and ends in the project.
...
Examples of recomposed patterns are shown in Figure
\ref{figure:patterns_plots}.
...
a maximum LOC difference less than 5,250; and 25\% less than 1,046. Hypothesis
\ref{hyp:pattern_low_diff} is invalid for \emph{all }\rm patterns, but might be
true for patterns of type A.
...
dead project. This means that patterns overlap even though all 25 patterns do
last until the end of the evolution of a project, but some start earlier.
