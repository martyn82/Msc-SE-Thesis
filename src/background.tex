...
To be able to analyse and compare wavelets of different lengths and scales, we
need a way to transform the signal. Discrete wavelet transformation is the
...
advantage over Fourier analysis that it can deal with 'choppy' signals, as
opposed to only sines and cosines \cite{graps}.
...
During wavelet transform, the signal is shifted and filtered.
Shifting/translating is the operation of moving the wavelet in the time domain
producing scale/filter coefficients, and requires frequency resolution.
Filtering/dilating is the operation of scaling the wavelet in the frequency
...
of the original signal allows, that is, as long as the number of resulting
coefficients is larger than 2 (filter length). Wavelet transforms using Haar
filter is illustrated in Figure \ref{figure:haar_transform}.
...
\paragraph{}
The resulting coefficients should only be compared to coefficients of the same
type. The scale/filter coefficients are incomparable to the wavelet/shift
coefficients, because that would mean comparing time values with frequency
values.
